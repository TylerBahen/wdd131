...
function displayResults(results) {
    ...
    results.forEach(recipe => {
        var tagBox = ''
        recipe.tags.forEach(tag => tagBox+=`<span>${tag}</span>`)
        var starBox = ''
        for (var i = 0; i < recipe.rating; i++) {
            starBox+=`<span aria-hidden="true" class="icon-star">⭐</span>`
        }
        while (i<5){
            i++
            starBox+=`<span aria-hidden="true" class="icon-star-empty">☆</span>`
        }
        out+=`<div class="recipe-card">
...
function search(){
    const params = document.getElementById('search').value.toLocaleLowerCase()
    const results = recipes.filter(recipe => recipe.name.toLocaleLowerCase().includes(params))
